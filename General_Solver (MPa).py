...
def solve():
    ...
    if dataT and dataC:
        ...
        steel_areaT = nT.astype(float)*(np.pi/4)*np.power(diamT.astype(float), 2)
        steel_areaC = nC.astype(float)*(np.pi/4)*np.power(diamC.astype(float), 2)
        ...
        stress_Tension = 600*((depthT.astype(float)-c_val) / c_val)
        stress_Compression = 600*((c_val-depthC.astype(float)) / c_val)
        stressT = []
        stressC = []
        if any(stress_Tension < 0):
            ...
            for i in range(len(stress_Tension)):
                if stress_Tension[i] < f_yBeam:
                    stressT.append(600*((float(depthT[i])-x)/x))
                if stress_Tension[i] >= f_yBeam:
                    stressT.append(f_yBeam)

            for i in range(len(stress_Compression)):
                if stress_Compression[i] < f_yBeam:
                    stressC.append(600*((x-float(depthC[i]))/x))
                if stress_Compression[i] >= f_yBeam:
                    stressC.append(f_yBeam)
            ...
            for i in range(len(stress_Tension)):
                if stress_Tension[i] < f_yBeam:
                    stressT.append(600*((float(depthT[i])-c_val)/c_val))
                if stress_Tension[i] >= f_yBeam:
                    stressT.append(f_yBeam)
            ...
            for i in range(len(stress_Compression)):
                if stress_Compression[i] < f_yBeam:
                    stressC.append(600*((c_val-float(depthC[i]))/c_val))
                if stress_Compression[i] >= f_yBeam:
                    stressC.append(f_yBeam)


        # Solving for d
        stressT = np.array(stressT, dtype=float)
        stressC = np.array(stressC, dtype=float)
        depthBeamT = np.sum(stressT*depthT.astype(float)) / np.sum(stressT)
    ...
    label_1.config(text=np.round(max_capacity/np.power(10, 6), 10))
    label_2.config(text=np.round(capacity/np.power(10, 6), 10))
...
def editSteel():
    ...
    trv4.heading(1, text='Depth (mm)')
    trv4.heading(2, text='No. of Bars')
    trv4.heading(3, text='Diameter (mm)')
...
mainWindow = Tk()
mainWindow.title("Rectangular Beam Capacity Solver (Metric Units)")
...
trv1.heading(1, text='Depth (mm)')
trv1.heading(2, text='No. of Bars')
trv1.heading(3, text='Diameter (mm)')
...
trv2.heading(1, text='Depth (mm)')
trv2.heading(2, text='No. of Bars')
trv2.heading(3, text='Diameter (mm)')
...
Label(wrapper1, text="mm").place(x=210, y=10)
Label(wrapper1, text="Mpa").place(x=210, y=40)
Label(wrapper1, text="Mpa").place(x=210, y=70)
...
Label(wrapper4, text="kN-m").place(x=430, y=10)
Label(wrapper4, text="kN-m").place(x=430, y=30)
Label(wrapper4, text="mm").place(x=430, y=90)
Label(wrapper4, text="mm").place(x=430, y=110)
Label(wrapper4, text="mm²").place(x=430, y=130)
Label(wrapper4, text="mm²").place(x=430, y=150)
Label(wrapper4, text="mm").place(x=430, y=170)
Label(wrapper4, text="mm").place(x=430, y=190)
